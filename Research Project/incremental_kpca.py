...
class IncrKPCA(object):
    ...
    X : numpy.ndarray, 2d
        Data matrix
    m0 : int
        Initial size of kernel matrix (or Nystr?m subset)
    mmax : int
        Maximum size of kernel matrix (or Nystr?m subset)
    kernel : callable
        Kernel function
    adjust : bool
        Whether to adjust the mean
    nystrom : bool
        Calculate incremental Nystr?m approximation instead
    maxiter : int
        Maximum number of iterations
    Yields
    ------
    The iteration number, the calculated eigenvectors and eigenvalues, and in
    the case of the Nystr?m method, also the approximate eigenvalues and
    eigenvectors
    """
...
def nystrom_approximation(L, U, K_nm):
    """
    Create the Nystr?m approximations to the eigenpairs of the kernel matrix
    Parameters
    ----------
    L : numpy.ndarray, 2d
    ...
    Returns
    -------
    Nystr?m approximate eigenvalues L and eigenvectors U
    """
    n, m = K_nm.shape
    L_nys = n/m * L
